# 🧠 100 Days of ML — Rimsha's Learning Journal

> *"The best way to learn data science is to do data science — every single day."*

[![Days Completed](https://img.shields.io/badge/Days%20Completed-42%2B-blueviolet?style=for-the-badge)](.)
[![Language](https://img.shields.io/badge/Language-Python-yellow?style=for-the-badge&logo=python)](.)
[![Tools](https://img.shields.io/badge/Tools-Pandas%20%7C%20NumPy%20%7C%20Seaborn%20%7C%20Sklearn-blue?style=for-the-badge)](.)
[![Status](https://img.shields.io/badge/Status-Active%20%F0%9F%94%A5-brightgreen?style=for-the-badge)](.)
[![Notebooks](https://img.shields.io/badge/Notebooks-42%2B-orange?style=for-the-badge)](.)

---

## 👩‍💻 About This Repository

This is my **public learning journal** — a daily commitment to becoming a better data scientist.

Every notebook here represents a real session where I picked a concept, got my hands dirty with actual data, and built something I could look back on. No copy-paste tutorials. No shortcuts. Just consistent, hands-on practice.

**Who is this for?**
- 🌱 Beginners looking for clean, well-documented reference implementations
- 🔥 Anyone doing their own #100DaysOfML challenge
- 💼 Recruiters who want to see *how* someone learns, not just what they claim to know

---

## 📂 What's Inside

Notebooks are organized by topic. Each folder covers one area of data science — just browse the folder that interests you and open any notebook directly on GitHub to see the code and outputs.

| Folder | What You'll Find | Notebooks |
|--------|-----------------|-----------|
| 📊 `eda/` | Univariate analysis, bivariate analysis, full EDA on real datasets | 3 |
| ⚙️ `preprocessing/` | Feature scaling, normalization, encoding, date/time handling | 7 |
| 🌐 `data-collection/` | Fetching data from REST APIs, web scraping pipelines | 2 |
| 🗄️ `data-handling/` | Working with CSV, JSON, SQL and pandas profiling | 3 |
| 🤖 `ml-models/` | Titanic survival prediction, ML pipelines with and without sklearn | 4 |

> 📌 New notebooks are pushed regularly as I progress through the challenge.

---

## 🗂️ Topics Covered So Far

```
✅ Exploratory Data Analysis (EDA)
✅ Univariate & Bivariate Analysis
✅ Feature Scaling & Standardization
✅ Normalization (Min-Max Scaling)
✅ Ordinal Encoding
✅ One Hot Encoding (OHE)
✅ Function Transformer
✅ Handling Mixed & DateTime Variables
✅ REST API Data Fetching
✅ Web Scraping with BeautifulSoup
✅ Working with CSV, JSON & SQL
✅ Pandas Profiling (Automated EDA)
✅ ML Pipelines (Titanic Dataset)
🔄 More coming daily...
```

---

## 🛠️ Tech Stack

```python
# Core
import pandas as pd
import numpy as np

# Visualization
import matplotlib.pyplot as plt
import seaborn as sns

# Machine Learning & Preprocessing
from sklearn.preprocessing import StandardScaler, MinMaxScaler, LabelEncoder, OrdinalEncoder
from sklearn.pipeline import Pipeline
from sklearn.model_selection import train_test_split

# Data Collection
import requests
from bs4 import BeautifulSoup

# Profiling
from ydata_profiling import ProfileReport
```

---

## 🚀 How to Run These Notebooks

```bash
# 1. Clone the repo
git clone https://github.com/rimsha-deve/100-days-of-ml.git
cd 100-days-of-ml

# 2. Install dependencies
pip install pandas numpy matplotlib seaborn scikit-learn requests beautifulsoup4 ydata-profiling

# 3. Launch Jupyter
jupyter notebook
```

Then open any folder and start exploring! ✅

---

## 🔗 Connect With Me

If you're also doing #100DaysOfML, let's connect and keep each other accountable!

[![GitHub](https://img.shields.io/badge/GitHub-rimsha--deve-black?style=flat-square&logo=github)](https://github.com/rimsha-deve)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-rimsha--shaukat--ai-blue?style=flat-square&logo=linkedin)](https://linkedin.com/in/rimsha-shaukat-ai)

---

## ⭐ If This Helped You

Give this repo a star — it helps other learners find it and keeps me motivated to keep pushing every day!

---

*Started: 2025 | Status: Ongoing 🔥 | Location: Lahore, Pakistan 🇵🇰*
